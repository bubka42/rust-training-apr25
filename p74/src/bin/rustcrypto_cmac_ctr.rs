...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "dec" => {
            let mut iv = [0u8; 16]; // 16 bytes IV for AES-128 CTR
            let mut tag = [0u8; 16]; // 16 bytes tag for CMAC
